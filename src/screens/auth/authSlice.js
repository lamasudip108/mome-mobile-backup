...
export const forgotCustomerPassword = createAsyncThunk(
    'auth/forgot',
    (formData, {rejectWithValue}) => {
        return store(`api/customers/forgot-password-notification`, formData).then(response => response.data.data).catch(error => rejectWithValue(error?.response?.data || error));
    },
);
